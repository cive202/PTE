...
def normalize_timing(
    ...
    """Convert absolute phone duration to relative z-score from speaker's baseline.
    
    This allows scoring relative to the speaker's own speech patterns,
    not native speaker expectations.
    
    Args:
        phone_duration: Observed phone duration
        phone_type: "vowel" or "consonant"
        baseline: Speaker baseline dict from analyze_speaker_baseline()
        
    Returns:
        Z-score relative to speaker's baseline (not native model)
    """
    if phone_type == "vowel":
        expected = baseline.get("median_vowel_duration", 0.10)
        std = expected * 0.3  # 30% coefficient of variation
    elif phone_type == "consonant":
        expected = baseline.get("median_consonant_duration", 0.06)
        std = expected * 0.3
    else:
        # Unknown type, use average
        expected = (baseline.get("median_vowel_duration", 0.10) + 
                   baseline.get("median_consonant_duration", 0.06)) / 2
        std = expected * 0.3
