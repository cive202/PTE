...
def score_pronunciation_from_phones(
    ...
    for phone in phones:
        ...
        if baseline and accent_tolerant and normalize_timing and _is_vowel and _is_consonant:
            ...
            z = normalize_timing(duration, phone_type, baseline)
            z_scores.append(abs(z))
        else:
            # Use absolute timing (native model)
            expected_duration = EXPECTED_PHONE_DURATIONS.get(normalized_label, 0.08)
            expected_std = PHONE_DURATION_STD.get(normalized_label, 0.02)
